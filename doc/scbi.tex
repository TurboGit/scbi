...
\title{{\Huge S C B I} \\ {\large  A Build Driver} \\ User's Guide}
\author{Pascal Obry}
\date{June 2019}
...
\begin{description}
	\item[\code{\$PWD/.env}] The local environment found in the current working directory.

	\item[\code{\$SCBI\_DEFS/.env}] The default global environment used for all builds.
\end{description}

Also note that instead of the default \file{.env} a specific environment file can be specified with the \code{\ddash{}env=<name>} command line option. (\seeref{cmdline}).
...
\begin{description}
	\item[\code{\$PWD/.plan-<name>}] A local build plane found in the current working directory.
