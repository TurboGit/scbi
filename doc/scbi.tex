...
\begin{description}[font=\large\texttt]
	...
	\item[<module>-{[}common|<variant>{]}-env] \index{hook!env}Set the environment to use this module as a dependency. This is the exported environment. The environment must be set using \code{add-to-var} function.
	\begin{description}[font=\textit,style=standard]
		\item[parameter] \tabto{2cm} \code{\$PREFIX \$TARGET [\$VARIANT]}
		\begin{enumerate}
	...
	\item[<module>-{[}common|<variant>{]}-build-env] \index{hook!build-env}Set the environment needed to build this module. The environment must be set using \code{add-to-var} function. Those settings are not exported and only used for the module.
	\begin{description}[font=\textit,style=standard]
		\item[parameter] \tabto{2cm} \code{\$PREFIX \$TARGET [\$VARIANT]}
		\begin{enumerate}
			\item \code{add-to-var} <name> <value>
