...
\begin{description}[style=nextline]
	...
	\begin{description}[font=\texttt]
		\item[native] a variant name that can be defined to use the distribution package. This name \code{native} is mandatory in this case and requited when using the \code{auto} mode (see below). When a \code{native} variant is referenced the \code{build-depends} hook is not used.
	...
	\begin{description}[font=\texttt]
		\item[<VCS>] a reference to a VCS version (branch or a Git SHA-1 for example), a tar version.
...
\begin{description}
	...
	\item[\code{[ ... ] /variant:version}] To create a group.
\end{description}
...
\begin{description}
	\item[ {[} barrier] Start with a square bracket and an option barrier. The barrier is a condition starting with \code{=} or \code{/=} and the name of a variant. The group will be evaluated only if the barrier evaluate to true otherwise it will be skipped. If the barrier is not specified the group will always be evaluated (i.e. the barrier is by default TRUE).
