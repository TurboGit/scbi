...
A version specified for a meta-module is propagated to all aggregated modules and recursively to all aggregated meta-modules. Note that this specification is \emph{soft}, meaning that it won't override any specific version specified for a module anywhere in the build plan or on the command line.
...
\begin{description}
	...
	\begin{description}
		...
		Finally the barrier can have two groups of values, for a \code{=} condition only one of the group can match (OR) for the barrier to evaluate to TRUE. For a \code{/=} condition both groups must match (AND) for the barrier to evaluate to TRUE.
		...
		Modules listed in the group without variant and version will inherit the current variant and version (if any).

		\item[ {]} /{[}+-{]}variant:version ] Ends with \code{/[+-]variant:version} which is the variant and version to apply to all modules in the group. Both \code{variant} and \code{version} are optional. If the variant starts with a plus (+) it is added to all modules in the group instead of replacing it. If the variant starts with a minus (-) it is removed from all modules in the group instead of replacing it.
	\end{description}
...
\begin{description}[style=nextline]
	\item[<module>-{[}common|<variant>{]}-modules] \index{hook!modules}Lists the aggregated modules.
		...
		\end{enumerate}
	\end{description}
	\item[<module>-{[}common|<variant>{]}-propagate-version] \index{hook!propagate-version}Whether the meta-module version is to be propagated to aggregated modules.
		\begin{description}[font=\textit,style=standard]
			\item[parameter] \tabto{2cm} \code{\$PREFIX \$TARGET [\$VARIANT]}
			\item[return] \tabto{2cm} \code{true | false}
		\end{description}
