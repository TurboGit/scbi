...
\begin{description}
	...
	\begin{description}
		...
		\item[ {]} /{[}+{]}variant:version ] Ends with \code{/[+]variant:version} which is the variant and version to apply to all modules in the group. Both \code{variant} and \code{version} are optional. If the variant starts with a plus it is added to all modules in the group instead of replacing it.
	\end{description}
