...
\subsection{Native variants}
\label{native variant}

For a plug-in it is sometimes needed to have a native variant to use the OS libraries instead of building from sources. It is possible to do so by creating the variant hooks manually. All hooks must be nop action except the \code{depends} hooks which must list the OS libraries and possible the \code{env} hook which may need to define some environment variables.

The support routine \code{with-variant} can be used to automate this action. For example:
...
The first parameter is the variant name. The followings are the libraries and an optional environment section can be opened with \code{\ddash{}env}.
...
If the current module reference is \code{mod/mp}i the \code{cmake} command will be:
