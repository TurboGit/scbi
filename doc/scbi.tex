...
\title{{\Huge S C B I} \\
	{\large  A Build Driver v\version} \\
	User's Guide}
\author{Pascal Obry}
\date{December 2019}
...
\begin{description}[font=\texttt]
	...
	\item[\ddash{}list-externals={[}all|run|dev{]} ] Recursively list external dependencies. Either \code{all} or \code{dev} dependencies are listed or only the ones needed at runtime. Those last dependencies are computed automatically from the \code{dev} dependencies listed in plugin modules.
