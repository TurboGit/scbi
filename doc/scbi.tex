...
\newcommand{\des}[1]{\tabto*{5cm}\scriptsize\textit{: #1}}
...
SCBI is a Source Code Build Infrastructure.
...
A module is described in an SCBI plug-in script (one plug-in in one file). It can be referenced in different places (as dependency in other plug-ins, build plan, etc...) with the following format which is called a \emph{module reference}:

\code{[for\_module=]name[/variant][:version]}
\label{dev}

\begin{description}[style=nextline]
	\item[for\_module] The name of the plug-in depending on this plug-in reference. This is optional and can be used to select a specific variant or version when the plug-in \code{name} is used as a dependency on \code{for\_module}. When specified on a build plan it is possible to use an alternative syntax, see \code{@for} \seeref{formodule}.

	\item[name] The name of the plug-in. This is the main reference for the plug-in. The corresponding plug-in script must be named identically and located in \code{\$SCBI\_PLUGINS}.
	...
	\begin{description}[font=\texttt]
		\item[<ID>] The variant name. This will trigger specific hook routines from the plug-in scripts.
	...
	\begin{description}[font=\texttt]
		...
		\item[keep] \index{version!keep}To specify that the module sources should not be updated. So the \emph{setup, }\emph{configure}, \emph{build}, \emph{install} and \emph{wrapup} steps are not conducted and the module's will keep its current status as it was built last time. If the module didn't get built yet the \emph{keep} status is ignored.
		\item[skip] \index{version!skip}To specify that the module must be completely ignored. It is as if the plug-in was not present in the dependencies.
		\item[force] \index{version!force}To enable an otherwise skip module (possibly set to \emph{skip} in a build plan).
		\item[@] \index{version!@}This is only available on the command line. To specify that the actual version is the one described in the build plan. If not defined in the build plan, then the default version is used (e.g. \code{master} for Git) as if it was not specified.
	\end{description}

	The version can be set to \code{force} on the command line to tell that the plug-in should not be skipped. On the command line it is always possible to force a build of a module to a specific variant and/or version. But if one need to build the default variant or version it is required to pass the \code{force} as version.

	That is, the following command will never overwrite a build plan as it is a default reference to a plug-in:
...
\begin{description}[style=nextline]
	...
	\item[lcms2/fast:v2.0] Build \code{lcms2} \emph{fast} variant from repository using branch (or tag) \emph{2.0}.

	\item[lcms2/@:v2.0] Build \code{lcms2} using the variant as defined in the build plan and from repository using branch (or tag) \emph{2.0}.
	...
	\item[lcms2:skip] Skip \code{lcms2} from the build. This is especially useful when set on a build plan or the command line to specify that the plug-in is to be ignored.
\end{description}
...
\begin{description}[before=\let\item\myitem]
	...
	\item[SCBI\_CACHE\_DIR] A variable pointing to the build's cache dir. Setting this variable to \emph{no} disables the cache. The cache makes it possible to reuse a previous build instead of rebuilding a module. Default is \file{\$HOME/.cache/scbi}.
...
\label{discriminants}
\index{discriminants}
\index{plan|discriminants}

A list of discriminants are generated for each build. Those discriminants can be used in barrier (\seeref{plan-barrier}). The discriminants are host and target specific and can be used to tune build plans.
...
\begin{description}
	...
	\item[\code{@ref m2 as m1}] To propagate a variant/version from another module.

	\begin{lstlisting}
		module1/var:1.2
		@ref module2 as module1
	\end{lstlisting}

	Here \code{module2} will use the variant and version of \code{module1}. The above plan is equivalent to:
	...
	But this may come handy when inheriting of a plan to set reference of a plug-in inheriting from another one.
	...
	\item[\code{@for <plug-in> use <modref>}] To specify a specific variant and/or version of a plug-in when used as dependencies of a specific module.
	\label{formodule}
	...
	\begin{description}
		...
		The barrier can have multiple variants or discriminants separated with a comma:

		\code{= deb,9}

		All of them must be set for the barrier to be true.

		Finally the barrier can have two groups of values. For a \code{=} condition only one of the group can match (OR) for the barrier to evaluate to TRUE. For a \code{/=} condition all groups must match (AND) for the barrier to evaluate to TRUE. Examples:
...
\begin{description}[font=\texttt]
	...
	\item[\ddash{}clear-cache] Clear the module's plan cache to force a full recomputation of auto module and meta-module module plan propagation. This is done automatically in many cases but some are not possible or too costly to detect automatically. The case taken into account are:
	\begin{itemize}
		\item Plan has changed (name and/or content),
		\item Some packages have been installed/removed from OS using \code{apt}, \code{dnf}, \code{pacman},
		\item Some OS discriminants have changed (name, version, etc.),
		\item The modules passed on the command line have their reference changed,
		\item The build target has changed.
	\end{itemize}

	\item[-w | \ddash{}wrapup] Do the wrapup step (default).

	\item[\ddash{}archive] Check for archives only if explicitly specified in the module reference. If this option is not specified an archive for the plug-in will be search into the archive cache directory (\file{\$SCBI\_BDIR/.archives}).
...
\begin{description}[font=\large\texttt]
	...
	\item[<module>{[}-<variant>{]}-vcs] \index{hook!vcs}Describe the module source VCS.
	...
	\item[<module>{[}-<variant>{]}-archive] \label{hookarchive}\index{hook!archive}Describe the archive filename and location of the archive containing the module sources.
	\begin{description}[font=\textit,style=standard]
	...
	\item[<module>{[}-<variant>{]}-version] \index{hook!version}Returns the current version of the installed module.
	\begin{description}[font=\textit,style=standard]
	...
	\item[<module>{[}-<variant>{]}-setup] \index{hook!setup}To put in place the sources just before configuration step. This is generally not needed as the sources are setup by \code{scbi} using the information provided by the archive or vcs hook. This may be needed in some complex case or if the sources are generated based on the running environment.
	\begin{description}[font=\textit,style=standard]
		\item[parameter] \tabto{2cm} \code{\$PREFIX \$TARGET \$VARIANT}
		\item[return] \tabto{2cm} NONE
	\end{description}

	\item[<module>{[}-<variant>{]}-out-of-tree] \index{hook!out-of-tree}Whether to build out of the source tree or not. Default is \code{true}.
	\begin{description}[font=\textit,style=standard]
		\item[parameter] \tabto{2cm} \code{\$PREFIX \$TARGET \$VARIANT}
	...
	\item[<module>{[}-<variant>{]}-only-explicit-build] \index{hook!only-explicit-build}Whether to build when found in dependencies or only when the plug-in is passed on the command line. If function returns \code{true} then it will be built only when on the command line. In all cases it will be used to get the environment when found in dependencies. Default is \code{false}.
	\begin{description}[font=\textit,style=standard]
		\item[parameter] \tabto{2cm} \code{\$PREFIX \$TARGET \$VARIANT}
	...
	\item[<module>{[}-<variant>{]}-config] \index{hook!config}To configure the sources before starting the build. This is generally a call to GNU \code{configure} or \code{cmake}. The source tree is always accessible as \code{../src}.
	\begin{description}[font=\textit,style=standard]
		\item[parameter] \tabto{2cm} \code{\$PREFIX \$TARGET \$VARIANT}
		\begin{enumerate}
	...
	\item[<module>{[}-<variant>{]}-build] \index{hook!build}The command to build the application. This is most of the case a simple call to \code{make}.
	\begin{description}[font=\textit,style=standard]
		\item[parameter] \tabto{2cm} \code{\$PREFIX \$TARGET \$VARIANT}
		\begin{enumerate}
			\item make
	...
	\item[<module>{[}-<variant>{]}-install] \index{hook!install}The command to install the application. This is most of the case a simple call to \code{make} with target \code{install}.
	\begin{description}[font=\textit,style=standard]
		\item[parameter] \tabto{2cm} \code{\$PREFIX \$TARGET \$VARIANT}
		\begin{enumerate}
			\item make install
	...
	\item[<module>{[}-<variant>{]}-wrapup] \index{hook!wrapup}This is to do whatever is needed after the installation step.
	\begin{description}[font=\textit,style=standard]
		\item[parameter] \tabto{2cm} \code{\$PREFIX \$TARGET \$VARIANT}
		\item[return] \tabto{2cm} NONE
	\end{description}

	\item[<module>{[}-<variant>{]}-prefix] \index{hook!prefix}The directory for the installation of the module. The returned value can be set to NONE to skip the installation step. If this hook is not defined the module will be installed into \code{\$SCBI\_PREFIX}.
	...
	\item[<module>{[}-<variant>{]}-tests] \index{hook!tests}Set of commands to run the tests for the module.
	\label{tests-hook}
	\begin{description}[font=\textit,style=standard]
	...
	\item[<module>{[}-common|<variant>{]}-run] \index{hook!run}Set the command line to run when option \code{\ddash{}run} is used without argument (\seeref{cmd!run}).
...
\begin{description}[style=nextline]
	...
	\item[<module>{[}-<variant>{]}-propagate-version] \index{hook!propagate-version}\label{propagate-version}Whether the meta-module's version and/or variant are to be propagated to aggregated modules. Default is \code{version}.
		\begin{description}[font=\textit,style=standard]
			\item[parameter] \tabto{2cm} \code{\$PREFIX \$TARGET [\$VARIANT]}
			\item[return] \tabto{2cm} \code{\textbf{version} | variant | both | false}
		\end{description}

	\item[<module>{[}-<variant>{]}-aggregate] \index{hook!aggregate}\label{aggregate}If set to \code{direct} or \code{recursive} the meta-module becomes an aggregate meta-module. In this case artifacts of all modules listed in \code{modules} hook are copied into the aggregate meta-module and the environment of all aggregated modules is pointing to the aggregate module. With this option it is possible to group into a single place the artifacts of multiple modules. When \code{direct} is used only the modules listed in the \code{modules} hook are aggregated, if \code{recursive} is used all dependencies are also aggregated. Default is \code{false}.
	\begin{description}[font=\textit,style=standard]
	\item[parameter] \tabto{2cm} \code{\$PREFIX \$TARGET [\$VARIANT]}
	\item[return] \tabto{2cm} \code{direct | recursive | \textbf{false}}
	\end{description}

	\item[<module>{[}-<variant>{]}-prefix] \index{hook!prefix}\label{prefix}The directory for the installation of the meta-aggregate module. This hook is not used for standard meta modules. The returned value can be set to NONE to skip the installation step. If this hook is not defined the module will be installed into \code{\$SCBI\_PREFIX}.
	\label{global-install}
	\begin{description}[font=\textit,style=standard]
		\item[parameter] \tabto{2cm} \code{\$PREFIX \$TARGET \$VARIANT}
		\item[return] \tabto{2cm} string
...
\begin{description}
	\item[<module>{[}-<variant>{]}-plan] \index{hook!plan}Returns the build-plan to use.
	\begin{description}[font=\textit,style=standard]
		\item[parameter] \tabto{2cm} NONE
		\item[return] \tabto{2cm} string
		\begin{enumerate}
...
The \code{external-env} hook can be used to ensure that the \code{wrapup} hook is run consistently. That is, if the \code{wrapup} hook use some environment variables or enabled options (\seeref{enable-feature}) to generate a package or take any action when a change is made on aggregated modules one need to add in the \code{external-env} hook the used environment.

\subsection{User's defined final step}
\label{module-final-step}
\index{hook!module-final-step}

After a successful build of a module or aggregate meta-module a specific hook named \code{module-final-step} is called.

\begin{description}
	\item[module-final-step] Called after a successful build. Can be used to update an application packaging for example.
	\begin{description}[font=\textit,style=standard]
		\item[parameter] \tabto{2cm} \code{\$PREFIX \$TARGET \$VARIANT \$MODULE}
		\par \tabto{2cm} \code{\$DEPENDS\_KIND \$IS\_META \$IS\_AGGREGATED \$IS\_UP\_TO\_DATE}
		\begin{description}
		\item [\$PREFIX] The installation directory for the module.
		\item [\$TARGET] The build target for the module.
		\item [\$VARIANT] The variants for the module.
		\item [\$MODULE] The module plug-in's name.
		\item [\$DEPENDS\_KIND] The kind of dependency for the module, values are \code{default}, \code{build} or \code{tests}
		\item [\$IS\_META] Wether the module is standard meta or aggregate meta, values are \code{yes}, \code{no} or \code{aggregate}
		\item [\$IS\_AGGREGATED] Wether the module is aggregated into a meta-module, values are \code{yes} or \code{no}.
		\item [\$IS\_UP\_TO\_DATE] Wether the module is up-to-date or not, values are \code{yes} or \code{no}. A module may be up-to-date but not yet handled as the dependency kind was rejected.
		\end{description}
		\item[return] \tabto{2cm} NONE
\end{description}
\end{description}
...
\begin{lstlisting}
	function module-final-step()
	{
		...
		#  Hande only standard dependencies
		#  (skip build & tests dependencies)

		[[ $DEPENDS_KIND != default ]] && return

		local APP="/some/dir/$MODULE"

		#  Copy if not up-to-date or not yet copied

		[[ $IS_UP_TO_DATE == no ]] || [[ ! -d $APP ]] &&
		   rsync -a $PREFIX/. $APP
	}
\end{lstlisting}
...
\subsection{Checking dependency version}
\index{helpers!get-module-version}\index{get-module-verson}
\index{helpers!check-module-version}\index{check-module-verson}

Those two helpers can be used to check a version of a dependency in a plug-in. This can be used for example to apply a patch for a specific version of a dependency.

\begin{description}
	\item[get-module-version] Take the name of a plug-in as parameter, returns the version for this plug-in as described in the build plan.

	\begin{description}[font=\textit,style=standard]
		\item[parameter] \tabto{2cm} plug-in
		\item[return] \tabto{2cm} version
	\end{description}

	Example:
	\begin{lstlisting}[basicstyle=\footnotesize]
		if [[ $(get-module-version lib2) =~ "2.0" ]]; then
		   ...
		else
		   ...
		fi
	...
	\begin{lstlisting}[basicstyle=\footnotesize]
		local VER=$(get-module-version lib2)
	\end{lstlisting}

	\item[check-module-version] Check a plug-in version. Take a plug-in name, an operator and a version, returns status code different than 0 if expression is true. The operator can be \code{=}, \code{!=}, \code{>}, \code{<}, \code{>=} or \code{<=}.

	\begin{description}[font=\textit,style=standard]
		\item[parameter] \tabto{2cm} plug-in operator version
		\item[return] \tabto{2cm} NONE
	\end{description}

	Example:
	\begin{lstlisting}
		if check-module-version "lib2" ">" "1.9"; then
		   ...
		else
		   ...
		fi
...
\begin{lstlisting}
	inherit-plugin [--no-variant
	               |--hooks=<hook1,hook2>
	               |--custom-hooks=<hook1,hook2>
	               |--name=<name>] <name>
\end{lstlisting}

\begin{description}
	\item[\ddash{}no-variant] This option is to not inherit from any hook's variant. Only the default variants will be inherited.
	\item[\ddash{}hooks=<hook1,hook2>] Specify a set of hooks separated with comma to inherit from. Multiple \code{\ddash{}hooks} options can also be used. If not specified all SCBI supported hooks are inherited.
	\item[\ddash{}custom-hooks=<hook1,hook2>] Specify a set of custom hooks separated with comma to inherit from. Multiple \code{\ddash{}custom-hooks} options can also be used.
	\item[\ddash{}name=<name>] Specify the name to be used for the inherited plug-in instead of the default plug-in name. This makes it possible to inherit from multiple modules having the same hook and chain them manually.
...
\begin{description}[font=\texttt]
	...
	\item[\ddash{}rdepends] List all plug-ins having as dependency the plug-in passed on the command line. The variants if any are displayed around brackets and the kind of dependency (test or build) in parenthesis.
	...
	\item[\ddash{}build-plan:{[}history{][}=filter{]}] List the final build plan that will be used. This is after loading main plan, inheriting from other plan \code{@load} and resolving the command line modules specification. With the sub-option \code{history} the definition history, from inherited plan for example, is displayed. This is handy for complex plan definition as it helps tracking the setting history. If \code{filter} is given it must contain a comma separated list of plug-in and only those plug-ins will be listed in the output.
...
\begin{description}[font=\texttt]
	...
	\item[\ddash{}mode:{[}dev|sandbox|sb|tests|use{]}] Specify a mode for entering the shell. This controls the environment to be set and the directory in which the shell will be moved. The default mode is \code{use}.
	...
	\begin{description}[font=\texttt]
		...
		\item[use] The shell stays in the current working directory and setup the environment for the module to be used by another one. That is the given module is a dependency (and all dependents modules recursively) for the work to be done.
	\end{description}
...
In this section we will use the hands-on demos plug-ins:

\begin{description}
	\item[h-handson-lib] A plug-in building a library.
	\item[h-handson-app] A plug-in building an application depending on \code{h-handson-lib} library.
\end{description}

To install the hands-on plug-in locally:

\begin{shellcommand}
	\item git clone https://github.com/TurboGit/scbi-handson.git
	\item cd scbi-handson
	\item make
\end{shellcommand}
...
\code{SCBI} does support this scenario directly by allowing a user's checkout to be used instead of the sources in the sandbox. It is highly discouraged to modify the sources in the sandbox which should stay untouched by users. Beware, depending on the \code{SCBI} options, the modifications in the source may be overwritten, so lost.

The recommended steps are (we assume the current VCS for the module is Git):

\subsubsection{Checkout modules}

All modules that will be worked on must be checked-out.

\begin{shellcommand}
	\item cd \$HOME/dev
	\item git clone https://github.com/TurboGit/scbi-handson-lib.git
\end{shellcommand}

Let's switch to the \code{broken} branch for the demo:

\begin{shellcommand}
	\item cd scbi-handson-lib
	\item git checkout broken
\end{shellcommand}

As the clone is not done into \code{\$HOME/dev/repositories/git} the variable \code{SCBI\_GIT\_REPO} must be set to point to the root checkout directory, \seeref{build-env}.
...
\subsubsection{Build using the sources from checkout}

The version \code{dev} is used to tell \code{SCBI} to use the sources from the user's checkout instead of the sandbox one. Here we use the dev version for \code{scbi-handson-lib} that we have cloned above and we let \code{SCBI} use master branch of \code{h-handson-app} in the Git VCS.

\cmd{scbi --deps h-handson-lib:dev h-handson-app}

\begin{footnotesize}
	\begin{verbatim}
2024/10/07 15:40:42 : Building h-handson-lib [default] (dev)
2024/10/07 15:40:42 : native x86_64-linux-gnu
2024/10/07 15:40:42 : steps : setup config build install wrapup
2024/10/07 15:40:42 : config starting
2024/10/07 15:40:43 : config completed
2024/10/07 15:40:43 : build starting
[ 16%] Building CXX object CMakeFiles/hello.dir/hello/hello.cpp.o
/<...>/hello.cpp: In function ‘const char* hello_message()’:
/<...>/hello.cpp:4:5: error: ‘x’ was not declared in this scope
4 |     x "hello";
|     ^
/<...>/hello.cpp:5:1: warning: no return statement in function returning non-void [-Wreturn-type]
5 | }
| ^
make[2]: *** [CMakeFiles/hello.dir/build.make:76: CMakeFiles/hello.dir/hello/hello.cpp.o] Error 1
make[1]: *** [CMakeFiles/Makefile2:87: CMakeFiles/hello.dir/all] Error 2
make: *** [Makefile:136: all] Error 2
2024/10/07 15:40:43 : h-handson-lib (FAILED) build failed with error 2
2024/10/07 15:40:43 : h-handson-lib (FAILED) to reproduce:
2024/10/07 15:40:43 : h-handson-lib (FAILED) bash /<...>/<...>/logs/build.cmd
2024/10/07 15:40:43 : h-handson-lib (FAILED) use bash -x option for debug
\end{verbatim}
\end{footnotesize}

\subsubsection{Fix sources from checkout}

Edit \file{hello/hello.cpp} and replace \code{x} by \code{return}. Restart build:

\cmd{scbi --deps h-handson-lib:dev h-handson-app}

\begin{verbatim}
2024/10/07 15:44:05 : Building h-handson-lib [default] (dev)
2024/10/07 15:44:05 : native x86_64-linux-gnu
2024/10/07 15:44:05 : steps : setup config build install wrapup
2024/10/07 15:44:05 : config starting
2024/10/07 15:44:06 : config completed
2024/10/07 15:44:06 : build starting
2024/10/07 15:44:06 : build completed
2024/10/07 15:44:06 : install starting
2024/10/07 15:44:06 : install completed
2024/10/07 15:44:06 : skip global install
2024/10/07 15:44:06 : End Building h-handson-lib [default] (dev)

2024/10/07 15:44:06 : Building h-handson-app [default] (master)
2024/10/07 15:44:06 : native x86_64-linux-gnu
2024/10/07 15:44:06 : steps : setup config build install wrapup
2024/10/07 15:44:06 : config starting
2024/10/07 15:44:07 : config completed
2024/10/07 15:44:07 : build starting
2024/10/07 15:44:07 : build completed
2024/10/07 15:44:07 : install starting
2024/10/07 15:44:07 : install completed
2024/10/07 15:44:07 : skip global install
2024/10/07 15:44:07 : End Building h-handson-app [default] (master)
\end{verbatim}

\subsubsection{Commit fixed sources in checkout}

\begin{shellcommand}
	\item git commit -m "..." ...
\end{shellcommand}

And eventually push the fixed sources upstream.

\begin{shellcommand}
	\item git push
\end{shellcommand}

\subsubsection{Copy artifacts into main application}

If the changed module contains shared libraries used by an application. To test the fixed code one can simply copy the new installed module's artifacts into the proper location:

\begin{shellcommand}
	\item cp /<...>/h-handson-lib/install/.. /dir/for/testing
\end{shellcommand}
...
The following command will expose a build failure that we will fix using three different methods.

\cmd{scbi --deps --plan=broken h-handson-app}

\begin{footnotesize}
\begin{verbatim}
2024/09/19 13:23:30 : Building h-handson-lib [default] (broken)
2024/09/19 13:23:30 : needed by:
2024/09/19 13:23:30 :   - h-handson-app
2024/09/19 13:23:30 : native x86_64-linux-gnu
2024/09/19 13:23:30 : steps : build
2024/09/19 13:23:30 : build starting
[ 16%] Building CXX object CMakeFiles/hello.dir/hello/hello.cpp.o
/<...>/hello.cpp: In function ‘const char* hello_message()’:
/<...>/hello.cpp:4:5: error: ‘x’ was not declared in this scope
4 |     x "hello";
...
5 | }
| ^
make[2]: *** [CMakeFiles/hello.dir/build.make:76 : CMakeFiles/hello.dir/hello/hello.cpp.o] Erreur 1
make[1]: *** [CMakeFiles/Makefile2:87 : CMakeFiles/hello.dir/all] Erreur 2
make: *** [Makefile:136 : all] Erreur 2
2024/09/19 13:23:30 : h-handson-lib (FAILED) build failed with error 2
2024/09/19 13:23:30 : h-handson-lib (FAILED) to reproduce:
2024/09/19 13:23:30 : h-handson-lib (FAILED) bash /<...>/h-handson-lib/<...>/logs/build.cmd
2024/09/19 13:23:30 : h-handson-lib (FAILED) use bash -x option for debug
\end{verbatim}
\end{footnotesize}

\begin{quote}
	\emph{\textbf{WARNING}: When the fix is found it must be applied in the module's repository outside of the sandbox or it will be lost.}

	\emph{Restarting an SCBI command with a setup set (so without specifying a specific step like \ddash{config} or \ddash{build}) will reset all sources from the reference repository in the sandbox.}
\end{quote}
...
\begin{description}[font=\file,style=standard]
	...
	\item[build-plan] \tabto{3cm} The build plan with the variant and version of all modules.
	...
	\item[<plug-ins>] \tabto{3cm} The plug-ins needed for the build.
\end{description}
...
\begin{description}
	\item[The configuration step] \tabto{3cm}  \cmd{scbi \ddash{}config \ddash{}log:yes h-handson-lib}
	\item[The build step] \tabto{3cm} \cmd{scbi \ddash{}build \ddash{}log:yes h-handson-lib}
\end{description}

Let's reproduce the issue:

\cmd{scbi --build --plan=broken h-handson-lib}

\begin{footnotesize}
	\begin{verbatim}
		/<...>/h-handson-lib/<...>/src/hello/hello.cpp:4:5: error: ‘x’ was not declared in this scope
		4 |     x "hello";
		|     ^
	\end{verbatim}
\end{footnotesize}

The failure is because a bug in \code{hello.cpp}.

Edit \file{<...>/h-handson-lib/build/src/hello.cpp} and replace \code{x} by \code{return}. Then restart the build:

\cmd{scbi --build --plan=broken h-handson-lib}

\begin{verbatim}
2024/09/19 14:40:42 : Building h-handson-lib [default] (broken)
2024/09/19 14:40:42 : native x86_64-linux-gnu
2024/09/19 14:40:42 : steps : build
2024/09/19 14:40:43 : build starting
2024/09/19 14:40:43 : build completed
2024/09/19 14:40:43 : skip global install
2024/09/19 14:40:43 : End Building h-handson-lib [default] (broken)
\end{verbatim}

The compilation is now fixed using sources in the sandbox, be sure to apply the changes into the repository hosting the sources. For testing purpose you can then simply copy the artifacts (shared libraries for example) where the application is looking for them.

\subsubsection{Use generated scripts}
...
In case of failure \code{SCBI} output the command to use to replay:
...
As an example, the build command file \code{build.cmd} for \code{h-handson-lib} plug-in looks like this:

\begin{lstlisting}
# h-handson-lib-build
(
   cd <...>/h-handson-lib/x86_64-linux-gnu-default/build
   source <...>/.scbi.runtime
   [[ -f ../logs/env.cmd ]] && source ../logs/env.cmd
   [[ -f ../logs/build-plan ]] &&
   load-runtime-module-plan ../logs/build-plan

   source ../logs/h-handson-lib

   h-handson-lib-build <...>/h-handson-lib/<...>/install \
...
To replay just execute the script:

\cmd{bash /.../logs/build.cmd}

One can then edit the sources in the sandbox and launch the command above to build again.

Here just fix the syntax error by changing \code{x} by \code{return}.

\cmd{bash /.../logs/build.cmd}

This time the compilation complete without issue:

\begin{verbatim}
	[ 16%] Building CXX object CMakeFiles/hello.dir/hello/hello.cpp.o
	[ 33%] Linking CXX static library libhello.a
	[ 33%] Built target hello
	[ 66%] Built target hey
	[100%] Built target goodbye
\end{verbatim}

The compilation is now fixed using sources in the sandbox, be sure to apply the changes into the repository hosting the sources. For testing purpose you can then simply copy the artifacts (shared libraries for example) where the application is looking for them.

\subsubsection{Use scbi shell command}

Another option is to enter into a shell set with the proper environment to run the configuration and build command manually directly from the sandbox.

\cmd{scbi shell --mode:sandbox h-handson-lib}

This will enter a sub-shell into the sandbox's build directory. The shell is setup with the proper environment for the plug-in and all the dependencies. So to reproduce the issue just restart the build command which is here \code{make} (see \code{h-handson-lib} plug-in's build hook):
...
Edit \file{../src/hello/hello.cpp} and replace \code{x} by \code{return}. Restart the build:
...
When done you can exit the scbi's sub-shell:
...
\dirtree{%
	...
	.2 \textbf{[n]}\des{optional, simple number used in flat mode}.
	.2 \textbf{[.vcs]}.
	...
	.3 build-deps-<variant> \des{build id of all dependencies}.
	.3 src $\rightarrow$ \des{symlink to sources (vcs or archive)}.
	.3 archive $\rightarrow$ \des{symlink to extracted archive}.
	.3 \textbf{[archive-src]} \des{extract of archive}.
	.3 source-id \des{latest source-id (corresponding to build symlink)}.
	.3 source-id<variant> \des{version of the source (archive or vcs)}.
	.3 source-ref \des{the full source reference for QA purpose}.
	...
	.3 \textbf{[<target>-<variant>]} \des{directory, or symlink in flat mode}.
	.4 \textbf{[logs]} \des{commands \& output for each steps}.
	.4 \textbf{[build]} \des{build directory}.
	.4 source-id \des{sha1 to identify the sources}.
	.4 build-id \des{sha1 to identify the build}.
	.4 build-deps \des{build-id of all dependencies}.
	.4 \textbf{[install]} \des{installation directory}.
	.4 manifest \des{list of all files in install above}.
	.4 \textbf{[src]} \des{rsync of ../src}.
}
}
